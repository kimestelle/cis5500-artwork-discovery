...
export default function LazyTable({ route, columns, defaultPageSize, rowsPerPageOptions }) {
  ...
  const handleChangePageSize = (e) => {
    ...
    // TODO (TASK 19): set the pageSize state variable and reset the current page to 1
      ...
      <Table>
        ...
        <TableBody>
          {data.map((row, idx) =>
            <TableRow key={idx}>
              {
                // TODO (TASK 20): the next 3 lines of code render only the first column. Wrap this with another map statement to render all columns.
