import { Box, Typography, TextField, Dialog, DialogContent, DialogTitle, Button } from '@mui/material';
...
export default function Artists() {
  ...
  const funFacts = [
    { question: 'Learn who the top 10 artists are at the MET or MOMA! Choose one below.', id: 'topartists' },
    { question: "Find artists by the number of artworks they have made! More than 3? More than 4? Take your pick", id: 'timebymedium' },
    { question: "Looking for artists that share a lot in common? Find artists with a specific number of keywords in common!", id: 'numkeywords' },
    { question: "Want to find the popularity of an artists? Type their name and we'll give you a score!", id: 'popularity' },
    { question: "Read about the artists that truly dominate the MET and MOMA!", id: 'bios' },
  ];
  ...
  const handleSubmit = async (id) => {
    ...
    if (id === 'topartists') {
      endpoint = `http://${config.server_host}:${config.server_port}/topartists/${encodeURIComponent(value)}`;
    } else if (id === 'timebymedium') {
      endpoint = `http://${config.server_host}:${config.server_port}/learnartists/${encodeURIComponent(value)}`;
    } else if (id === 'numkeywords') {
      endpoint = `http://${config.server_host}:${config.server_port}/numkeywords/${encodeURIComponent(value)}`;
    } else if (id === 'popularity') {
      endpoint = `http://${config.server_host}:${config.server_port}/popularity/${encodeURIComponent(value)}`;
    } else if (id === 'bios') {
      endpoint = `http://${config.server_host}:${config.server_port}/bios/${encodeURIComponent(value)}`;
    }
  ...
  return (
    ...
    >
	  ...
	  <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
		  Result
		  <Button onClick={() => setOpenModal(false)} sx={{ minWidth: 'auto', fontWeight: 'bold' }}>
		  X
		  </Button>
	  </DialogTitle>
	  <DialogContent>
