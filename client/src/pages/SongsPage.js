...
export default function SongsPage() {
	...
	const search = () => {
		fetch(
			...
			.catch((err) => {
				console.error(err);
				alert(
					'An error occurred while searching for songs. Please try again later.'
				);
			});
	};
	...
	return (
		<Container>
			...
			<Button
				onClick={() => {
					const danceabilityLow = Math.random() * 0.7;
					const energyLow = Math.random() * 0.7;
					const valenceLow = Math.random() * 0.7;

					const danceabilityHigh =
						danceabilityLow + Math.random() * (1 - danceabilityLow);
					const energyHigh =
						energyLow + Math.random() * (1 - energyLow);
					const valenceHigh =
						valenceLow + Math.random() * (1 - valenceLow);

					setDanceability([danceabilityLow, danceabilityHigh]);
					setEnergy([energyLow, energyHigh]);
					setValence([valenceLow, valenceHigh]);
					search();
				}}
			>
				Surprise Me
